...
class CreateSellListings:
    ...
    def create_sell_listings(self, game_quantity: int) -> None:
        ...
        for idx, game_id in enumerate(game_ids):
            ...
            if self._manual:
                ...
                self._automatic_item_create_sell_listing(
                    games=games,
                    game_id=game_id,
                    inventory_summary=inventory_summary,
                )
    ...
    def _manual_item_create_sell_listing(self, games: SteamGames, game_id: int, inventory_summary: dict) -> None:
        ...
        SteamTraderUI.show_marketable_items_to_sell_summary(items=items, summary=tc_and_bp_summary)

        for item in items:
            item_id = item['item_id']
            if item_id not in inventory_summary.keys():
                continue
            ...
            asset_ids = self._steam_trader.inventory.get_asset_ids(
                item_id=item_id,
                marketable=True,
            )
    ...
    def _automatic_item_create_sell_listing(self, games: SteamGames, game_id: int, inventory_summary: dict) -> None:
        items = games.get_trading_cards_and_booster_pack(game_id=game_id, foil=False)
        tc_and_bp_summary = {
            item['item_id']: inventory_summary[item['item_id']]
            for item in items if item['item_id'] in inventory_summary.keys()
        ...
        for item in items:
            ...
            price = market_prices.recommended_seller_price()
            asset_ids = self._steam_trader.inventory.get_asset_ids(
                item_id=item_id,
                marketable=True,
                origin_undefined=False,
            )
            self._steam_trader.market_actions_queue.put((
                'create_sell_listing', {
                    'asset_ids': asset_ids,
